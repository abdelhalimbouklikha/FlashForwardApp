...
final class AppSettings: ObservableObject {
    ...
    // ---- v1.3 — Task 6. Review rating button order ----
    //
    // Persisted as a JSON-encoded `[Rating]` (we store the raw `Int`
    // values and rebuild the enum on load). The review screen renders the
    // four buttons (`Again`, `Hard`, `Good`, `Easy`) in this order, so the
    // user can reorder them via the Settings → Review "Boutons" UI.
    // Default is the canonical FSRS order.
    @Published var ratingButtonOrder: [Rating] {
        didSet { saveRatingOrder(ratingButtonOrder) }
    }
    ...
    init() {
        ...
        self.ratingButtonOrder = AppSettings.loadRatingOrder()
    }
    ...
    // MARK: - Rating button order (Task 6)

    /// Moves the given rating one slot up (toward the front / leftmost).
    /// No-op if it's already first.
    func moveRatingUp(_ rating: Rating) {
        guard let i = ratingButtonOrder.firstIndex(of: rating), i > 0 else { return }
        ratingButtonOrder.swapAt(i, i - 1)
    }

    /// Moves the given rating one slot down (toward the back / rightmost).
    /// No-op if it's already last.
    func moveRatingDown(_ rating: Rating) {
        guard let i = ratingButtonOrder.firstIndex(of: rating),
              i < ratingButtonOrder.count - 1 else { return }
        ratingButtonOrder.swapAt(i, i + 1)
    }

    /// Restores the canonical FSRS order: Again, Hard, Good, Easy.
    func resetRatingButtonOrder() {
        ratingButtonOrder = [.again, .hard, .good, .easy]
    }

    private func saveRatingOrder(_ order: [Rating]) {
        // Normalize: ensure all four ratings are present exactly once so
        // a corrupted store can never drop a button from the UI.
        let normalized: [Rating] = {
            var seen = Set<Rating>()
            var out: [Rating] = []
            for r in order where !seen.contains(r) {
                out.append(r); seen.insert(r)
            }
            // Append any missing ratings in canonical order.
            for r in [.again, .hard, .good, .easy] as [Rating] where !seen.contains(r) {
                out.append(r); seen.insert(r)
            }
            return out
        }()
        let raw = normalized.map(\.rawValue)
        if let data = try? JSONEncoder().encode(raw),
           let s = String(data: data, encoding: .utf8) {
            UserDefaults.standard.set(s, forKey: "ratingButtonOrder")
        }
    }

    private static func loadRatingOrder() -> [Rating] {
        // Default canonical order.
        let fallback: [Rating] = [.again, .hard, .good, .easy]
        guard let s = UserDefaults.standard.string(forKey: "ratingButtonOrder"),
              let data = s.data(using: .utf8),
              let raw = try? JSONDecoder().decode([Int].self, from: data) else {
            return fallback
        }
        let decoded = raw.compactMap { Rating(rawValue: $0) }
        // Rebuild missing ones so the array always contains 4 distinct
        // ratings (defensive against tampered UserDefaults).
        var seen = Set(decoded)
        var out = decoded
        for r in fallback where !seen.contains(r) { out.append(r); seen.insert(r) }
        return out
    }
